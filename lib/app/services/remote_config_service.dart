...
class RemoteConfigService extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    if (db == null) {
      if (kDebugMode) {
        AppLog.info(
          'RemoteConfigService: Firebase unavailable, continuing with defaults',
        );
      }
      return;
    }
    ...
    _appConfigSub ??=
        db.collection('config').doc('app_config').snapshots().listen((snap) {
      if (snap.exists) {
  ...
  FirebaseFirestore? get _dbOrNull {
    ...
    try {
      ...
      if (kDebugMode) {
        AppLog.error('RemoteConfigService: Firestore unavailable: $e', s);
      }
      return null;
    }
  }
