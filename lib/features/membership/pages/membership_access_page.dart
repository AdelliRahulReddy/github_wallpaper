...
class _MembershipAccessPageState extends State<MembershipAccessPage> {
  ...
  MembershipInfo get _info =>
      _membershipInfo ??
      context.read<MembershipController>().info ??
  ...
  Future<void> _refreshPageState({
    bool forceMembershipRefresh = true,
    ...
    try {
      var customerInfo = seededCustomerInfo;
      if (_billingAvailable && customerInfo == null) {
      ...
      setState(() {
        ...
        _activeProEntitlement =
            RevenueCatService.activeProEntitlementFromCustomerInfo(
          customerInfo,
        );
      ...
      AppLog.error('Membership access refresh failed: $error', stackTrace);
      if (!mounted) return;
      final message =
