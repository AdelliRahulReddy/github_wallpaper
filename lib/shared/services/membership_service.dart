...
class MembershipService {
  ...
  static Future<MembershipInfo> refresh({bool force = false}) async {
    ...
    if (email == null) {
      final fallback = (cached ?? MembershipInfo.free()).copyWith(
    ...
    try {
      ...
      final existing = snapshot.exists
          ? _membershipFromFirestore(snapshot.data() ?? const <String, dynamic>{})
          : null;
      ...
      resolved = await _mergeRevenueCatEntitlements(resolved);
      ...
      if (cached != null) {
        final fallback = cached.copyWith(fromCache: true);
        await StorageService.setCachedMembershipInfo(fallback);
        return fallback;
      }
      final fallback = MembershipInfo.free(
        lastValidatedAt: now,
        fromCache: true,
      );
      await StorageService.setCachedMembershipInfo(fallback);
